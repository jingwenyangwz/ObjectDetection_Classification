 ...
 */
 void floodFill(int x, int y, Mat& aux, int& maxX, int& maxY, int& minX, int& minY){
	 if(x >= 0 && y >= 0 && x < aux.rows && y < aux.cols && aux.at<int>(x,y) == 255){
		 ...
		 floodFill(x-1,y-1,aux, maxX, maxY, minX, minY);
		 floodFill(x,y-1,aux, maxX, maxY, minX, minY);
		 floodFill(x+1,y-1,aux, maxX, maxY, minX, minY);

		 floodFill(x-1,y,aux, maxX, maxY, minX, minY);
		 floodFill(x+1,y,aux, maxX, maxY, minX, minY);

		 floodFill(x-1,y+1,aux, maxX, maxY, minX, minY);
		 floodFill(x,y,aux, maxX, maxY, minX, minY);
		 floodFill(x+1,y+1,aux, maxX, maxY, minX, minY);
	 }
 }
...
{	
	...
	//cv::Mat img = cv::Mat::zeros(aux.cols+2,aux.rows+2,CV_8UC1);
	...
	for(int i = 0; i < fgmask.rows; i++){
		for(int j = 0; j < fgmask.cols; j++){
			if(aux.at<int>(i,j) == 255){
				...
				int maxY = j;
				int minY = j;
				floodFill(i, j, aux, maxX, maxY, minX, minY);
				cvBlob blob=initBlob(counter, minY, minX, maxY-minY, maxX-minX);
				bloblist.push_back(blob);
 ...
 int classifyBlobs(std::vector<cvBlob> &bloblist)
 {
	...
	int th = 1.4;
 	...
 	for(unsigned int i = 0; i < bloblist.size(); i++)
 	{
 		...
 		//...
 		float aspectRatio = float(blob.w)/float(blob.h);
 		//std::cout << "the ratio"  << float(blob.w)/float(blob.h) << std::endl;
 		float dist_person = ED(aspectRatio,MEAN_PERSON);
 		float dist_car = ED(aspectRatio,MEAN_CAR);
 		float dist_object = ED(aspectRatio,MEAN_OBJECT);
 		//I dont know why this is not good
 		if(( (aspectRatio > MEAN_PERSON - STD_PERSON)
 				&& (aspectRatio < MEAN_PERSON + STD_PERSON))
 				&& (dist_person <= min(dist_car, dist_object)) ){
 			blob.label = PERSON;
 		}else if(( (aspectRatio > MEAN_CAR - STD_CAR)
 				&& (aspectRatio < MEAN_CAR + STD_CAR))
 				&& (dist_car <= min(dist_person, dist_object))){
 			blob.label = CAR;
 		}else if(( (aspectRatio > MEAN_OBJECT - STD_OBJECT)
 				&& (aspectRatio < MEAN_OBJECT + STD_OBJECT))
 				&& (dist_object <= min(dist_car, dist_person))){
 			blob.label = OBJECT;
 		}else{
 			blob.label = UNKNOWN;
 		}

 		//std::cout << WED(aspectRatio, MEAN_PERSON, STD_PERSON) << std::endl;
 		// I dont know what kind of threshold would be appropriate?

 		/*if( WED(aspectRatio, MEAN_PERSON, STD_PERSON) < th){
 			std::cout << "ember";
 			blob.label = PERSON;
 		}else if(WED(aspectRatio, MEAN_CAR, STD_CAR) < th){
 			blob.label = CAR;
 		}else if(WED(aspectRatio, MEAN_OBJECT, STD_OBJECT) < th){
 			blob.label = OBJECT;
 		}else{
 			blob.label = UNKNOWN;
 		}*/
